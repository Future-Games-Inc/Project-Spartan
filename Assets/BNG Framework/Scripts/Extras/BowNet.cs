...
namespace BNG {
    ...
    /// </summary>
    public class BowNet : GrabbableEvents {
        ...
        void Update() {
            ...
            if (canGrabArrowFromKnock() && currentArrows >0) {
                ...
                GrabArrow(arrow.GetComponent<ArrowNet>());

                arrow.gameObject.GetComponent<ArrowNet>().arrowOwner = player.gameObject;
            }
        ...
        public void GrabArrow(ArrowNet arrow) {

            arrowGrabber = ClosestGrabber;

            // Signal the grabbable that we're being held
            GrabbedArrow = arrow.GetComponent<ArrowNet>();
