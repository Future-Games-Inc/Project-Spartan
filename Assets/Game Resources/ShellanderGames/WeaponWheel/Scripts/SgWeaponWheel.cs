...
namespace ShellanderGames.WeaponWheel
{
	...
	public class SgWeaponWheel : MonoBehaviour
	{
	   ...
	   public void RemoveEventCallback(Action<SgWeaponWheelEvent> action)
		...
		private void AnimateVisibility(bool visible)
		{
			...
			if(visible) {
				DehighlightAll(true);
				generatedObjectsContainer.gameObject.SetActive(true);
			}
		...
		private void AnimateVisibilityDone()
		{
			...
			generatedObjectsContainer.gameObject.SetActive(newIsVisible);
		}
	...
	public class SgWeaponWheelInspector : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			//Fold out sections
			DrawFoldOut(m_GeneralSettingsSection);
			DrawFoldOut(m_SlicesSection);
			DrawFoldOut(m_CirclesSection);
			DrawFoldOut(m_SliceBordersSection);
			DrawFoldOut(m_ResoursesSection);
		...
		private class SgSliceBordersSection : SgFoldoutSection
		{
			public SgSliceBordersSection(SgWeaponWheel weaponWheel, SerializedObject serializedObject)
			{
				...
				//"autoRefresh", "fadeInDuration", "fadeOutDuration", "sliceBorderWidth", 
				//"sliceBorderColors", "sliceBorderCutInnerAdjustment", "sliceBorderCutOuterAdjustment", "cutCirclesToo",
				//	"sliceContentColors", "rotation", "sliceContents", "logSelectionEvents"
			}
